...
export const List = ({ booking, showDate }: { booking: CLASSROOMBOOKING; showDate?: true }) => {
	...
	return (
		<li
			...
			className={`grid ${
				showDate ? 'grid-cols-9' : 'grid-cols-7'
			...
			<div className='w-full flex flex-col gap-2 items-start col-span-2'>
				...
				<p className='font-medium text-gray-400 flex items-center text-center justify-center w-full h-full'>{getUserFromBookings(booking.userId)}</p>
			</div>
		</li>
	);
};
